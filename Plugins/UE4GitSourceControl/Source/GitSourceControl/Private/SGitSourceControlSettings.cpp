		...
		[
			...
			[
				...
				[
					...
					//.Directory(GetRepositoryPathString())
					//.OnDirectoryChanged(this, &SGitSourceControlSettings::OnRepositoryRootPicked)
				]
			]
			// User Name
			+SVerticalBox::Slot()
			...
			[
				...
				[
					SNew(SCheckBox)
					.IsChecked(SGitSourceControlSettings::CanUseGitLfsLocking())
					.OnCheckStateChanged(this, &SGitSourceControlSettings::OnCheckedUseGitLfsLocking)
					.IsEnabled(this, &SGitSourceControlSettings::DisableCheckBox)
				]
				+SHorizontalBox::Slot()
				.FillWidth(0.9f)
				.VAlign(VAlign_Center)
...
FString SGitSourceControlSettings::GetRepositoryPathString() const
{
	FGitSourceControlModule& GitSourceControl = FModuleManager::GetModuleChecked<FGitSourceControlModule>("GitSourceControl");
	FString RepoPath = GitSourceControl.AccessSettings().GetRepositoryRootPath();
	if (RepoPath.IsEmpty()) {
		RepoPath = GitSourceControl.GetProvider().GetPathToRepositoryRoot();
		GitSourceControl.AccessSettings().SetRepositoryRootPath(RepoPath);
		GitSourceControl.AccessSettings().SaveSettings();
	}
	return RepoPath;
}

void SGitSourceControlSettings::OnRepositoryRootPicked(const FString & PickedPath) const
{
	FGitSourceControlModule& GitSourceControl = FModuleManager::GetModuleChecked<FGitSourceControlModule>("GitSourceControl");
	FString PickedFullPath = FPaths::ConvertRelativePathToFull(PickedPath);
	const bool bChanged = GitSourceControl.AccessSettings().SetRepositoryRootPath(PickedFullPath);
	if (bChanged)
	{
		GitSourceControl.AccessSettings().SaveSettings();
	}
...
bool SGitSourceControlSettings::CanInitializeGitRepository() const
{
	...
	//const bool bGitLfsConfigOk = bIsUsingGitLfsLocking;
...
bool SGitSourceControlSettings::DisableCheckBox() const
{
	return false;
}
...
{
	FGitSourceControlModule& GitSourceControl = FModuleManager::GetModuleChecked<FGitSourceControlModule>("GitSourceControl");
	bool UsingLfsLocking = GitSourceControl.AccessSettings().IsUsingGitLfsLocking();
	if (!UsingLfsLocking) {
		GitSourceControl.AccessSettings().SetUsingGitLfsLocking(true);
		GitSourceControl.AccessSettings().SaveSettings();
		UsingLfsLocking = true;
	}
	return false;
}
...
FText SGitSourceControlSettings::GetLfsUserName() const
{
	...
	FString UserName = GitSourceControl.GetProvider().GetUserName();
	if (LfsUserName != UserName) {
		LfsUserName = UserName;
		GitSourceControl.AccessSettings().SetLfsUserName(LfsUserName);
		GitSourceControl.AccessSettings().SaveSettings();
	}
