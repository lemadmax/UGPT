...
const FString FGitSourceControlSettings::GetRepositoryRootPath() const
{
	FScopeLock ScopeLock(&CriticalSection);
	return RepositoryRootPath; // Return a copy to be thread-safe
}
...
bool FGitSourceControlSettings::SetRepositoryRootPath(const FString & InString)
{
	FScopeLock ScopeLock(&CriticalSection);
	const bool bChanged = (RepositoryRootPath != InString);
	if (bChanged)
	{
		RepositoryRootPath = InString;
		//UE_LOG(LogSourceControl, Error, TEXT("setting lfs locks repository root: %s"), *RepositoryRootPath);
	}
	return bChanged;
}
...
void FGitSourceControlSettings::LoadSettings()
{
	...
	GConfig->GetString(*GitSettingsConstants::SettingsSection, TEXT("RepositoryPath"), RepositoryRootPath, IniFile);
}
...
void FGitSourceControlSettings::SaveSettings() const
{
	...
	GConfig->SetString(*GitSettingsConstants::SettingsSection, TEXT("RepositoryPath"), *RepositoryRootPath, IniFile);
}
