...
bool FGitSourceControlMenu::StashAwayAnyModifications()
{
	...
	//const FString& PathToRespositoryRoot = GitSourceControl.AccessSettings().GetRepositoryRootPath();
	...
	for (const auto& Sub : AllProjects) {
		FString RepoRoot = PathToRespositoryRoot + TEXT("/") + Sub;
		...
		const bool bStatusOk = GitSourceControlUtils::RunCommand(TEXT("status"), PathToGitBinary, RepoRoot, ParametersStatus, TArray<FString>(), InfoMessages, ErrorMessages);
		if ((bStatusOk) && (InfoMessages.Num() > 0))
		{
			...
			if (Choice == EAppReturnType::Ok)
			{
				...
				bStashMadeBeforeSync = GitSourceControlUtils::RunCommand(TEXT("stash"), PathToGitBinary, RepoRoot, ParametersStash, TArray<FString>(), InfoMessages, ErrorMessages);
...
void FGitSourceControlMenu::ReApplyStashedModifications()
{
	if (bStashMadeBeforeSync)
	{
		...
		for (const auto& Sub : AllProjects) {
			...
			TArray<FString> InfoMessages;
			TArray<FString> ErrorMessages;
			const bool bUnstashOk = GitSourceControlUtils::RunCommand(TEXT("stash"), PathToGitBinary, RepoRoot, ParametersStash, TArray<FString>(), InfoMessages, ErrorMessages);
			if (!bUnstashOk)
			{
				FMessageLog SourceControlLog("SourceControl");
